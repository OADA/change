...
export function applyChange(document: Document, change: Change): Document {
  ...
  for (const key in change) {
    ...
    if (utils.isObject(change[key])) {
      ...
      // Case 2: recursively apply change
        ...
        document[key] = applyChange(document[key], change[key]);
      }
    }
...
export function generateChange(document1: Document, document2: Document): Change {
  const change: Change = {};
  // Get union of keys
  ...
  for (const key of keys) {
    ...
    // The key exists in both documents
      ...
      if (utils.isObject(document1[key]) && utils.isObject(document2[key])) {
        change[key] = generateChange(document1[key], document2[key]);
      }
